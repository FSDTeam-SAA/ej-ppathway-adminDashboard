...
export function Modal({ open, onClose, title, children, size = "md", hideClose }: Props) {
  useEffect(() => {
    if (!open) return;
      ...
      <div className="text-center py-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            onClick={onClose}
            disabled={loading}
            className="h-11 rounded-lg border border-[#0a7a90] text-[#0a7a90] hover:bg-[#e6f2f6] font-medium disabled:opacity-50"
          >
            {cancelText}
          </button>
          <button
            ...
            className={`h-11 rounded-lg font-medium text-white disabled:opacity-50 ${
              danger ? "bg-red-600 hover:bg-red-700" : "bg-[#0a7a90] hover:bg-[#076377]"
            }`}
          >
            {loading ? "..." : confirmText}
          </button>
